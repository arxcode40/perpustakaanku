			...
			<div class="card-body">
				<div class="mb-3 row">
					...
					<div class="col-md-9 col-lg-10">
						<input class="form-control" disabled="disabled" id="lendingCode" type="text" value="<?= html_escape($lending['lending_id'])?>" />
					</div>
				</div>
				<div class="mb-3 row">
					...
					<div class="col-md-9 col-lg-10">
						<select autofocus="autofocus" class="form-select <?= form_error('name') === '' ?: 'is-invalid' ?>" id="memberName" name="name">
							...
							<?php foreach ($members as $member): ?>
								<option <?= set_select('name', html_escape($member['member_id']), html_escape($lending['member_id']) === html_escape($member['member_id'])) ?> value="<?= html_escape($member['member_id']) ?>"><?= html_escape($member['member_name']) ?></option>
							<?php endforeach ?>
						</select>
					...
					<div class="col-md-9 col-lg-10">
						<select class="form-select <?= form_error('title') === '' ?: 'is-invalid' ?>" id="bookTitle" name="title">
							...
							<?php foreach ($books as $book): ?>
								<option <?= set_select('title', html_escape($book['book_id']), html_escape($lending['book_id']) === html_escape($book['book_id'])) ?> value="<?= html_escape($book['book_id']) ?>"><?= html_escape($book['book_title']) ?></option>
							<?php endforeach ?>
						</select>
				...
				<div class="mb-3 row">
					...
					<div class="col-md-9 col-lg-10">
						<input class="form-control <?= form_error('start') === '' ?: 'is-invalid' ?>" id="lendingStart" name="start" type="date" value="<?= html_escape(set_value('start', $lending['lending_start'])) ?>" />
				...
				<div class="row">
					...
					<div class="col-md-9 col-lg-10">
						<input class="form-control <?= form_error('end') === '' ?: 'is-invalid' ?>" id="lendingEnd" name="end" type="date" value="<?= html_escape(set_value('end', $lending['lending_end'])) ?>" />
